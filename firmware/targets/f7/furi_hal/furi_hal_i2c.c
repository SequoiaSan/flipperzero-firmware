...
bool furi_hal_slave_i2c_rx(
    ...
    furi_check(handle->bus->current_handle == handle);
    furi_assert(timeout > 0);
    furi_assert(furi_hal_i2c_ownaddress1_external != 0);
    ...
    do {
        ...
        while (!LL_I2C_IsActiveFlag_STOP(handle->bus->i2c) || size > 0) {
            ...
            {
                if (LL_I2C_GetAddressMatchCode(handle->bus->i2c) == furi_hal_i2c_ownaddress1_external)
                {
                    printf("Address match!\n");
                    /* Verify the transfer direction, a write direction, Slave enters receiver mode */
                    if (LL_I2C_GetTransferDirection(handle->bus->i2c) == LL_I2C_DIRECTION_WRITE)
                    {
